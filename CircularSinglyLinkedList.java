...
{  
 ...
 Node nptr = new Node(val,null);   Node ptr = start;  
 pos = pos - 1 ;  
 for (int i = 1; i < size - 1; i++)   {  
 if (i == pos)  
 {  
 Node tmp = ptr.getLink() ;   ptr.setLink( nptr );  
 nptr.setLink(tmp);  
 break;  
 }  
 ptr = ptr.getLink();  
 ...
 {  
 start = start.getLink();   end.setLink(start);  
 size--;  
 return ;  
 }  
 }  
 ...
 public void display()  
 {  
 System.out.print("\nCircular Singly Linked List = ");   Node ptr = start;  
 if (size == 0)  
 {  
...
{  
 ...
 break;  
 case 3 :  
 System.out.println("Enter integer element to insert");   int num = scan.nextInt() ;  
